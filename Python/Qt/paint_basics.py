...
class MainWindow(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent):
        """Reimplement event handler to create a QPainter
        object that is used throughout the example."""
        painter = QPainter()
        painter.begin(self)
    ...
    def drawDiffLines(self, painter: QPainter):
        """Examples of how to draw lines with QPainter."""
        pen = QPen(QColor(self.red), 2)

        painter.setPen(pen)
        painter.drawLine(230, 20, 230, 20)
    ...
    def drawRectangles(self, painter: QPainter):
        ...
        # demonstrate how to change the alpha
        # channel to include transparency
        blue_pen = QPen(QColor(32, 80, 230, 100), 5)
        blue_pen.setStyle(Qt.PenStyle.DashLine)
        painter.setPen(blue_pen)
        painter.setBrush(green_brush)
        painter.drawRect(120, 320, 80, 80)

    def drawPolygons(self, painter: QPainter):
    ...
    def drawRoundedRects(self, painter: QPainter):
        """Examples of how to draw rectangles with rounded
        corners with QPainter"""
        pen = QPen(QColor(self.black))
        brush = QBrush(QColor(self.black))
        ...
        painter.setBrush(brush)
        painter.drawRoundedRect(rect_2, 8, 8)
    ...
    def drawCurves(self, painter: QPainter):
