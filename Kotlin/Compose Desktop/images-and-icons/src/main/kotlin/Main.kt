    ...
    DesktopMaterialTheme {
        Column {
            Row(modifier = Modifier.weight(1f)) {
                ...
                /**
                 * Loading an image from the internet or
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    AsyncImage(
                        load = { loadXmlImageVector(File("arm_flex_outline.xml"), density) },
                        painterFor = { rememberVectorPainter(it) },
                        contentDescription = "Arm Flex",
                        contentScale = ContentScale.FillWidth,
                        modifier = Modifier.width(200.dp)
                    )
                }
...
fun main() = application {
    ...
    // create tray icons for the app
    Tray(
        icon,
        menu = {
            Item("Balls are fun...", onClick = {  })
            Separator()
...
fun loadXmlImageVector(url: String, density: Density): ImageVector =
    URL(url).openStream().buffered().use { androidx.compose.ui.res.loadXmlImageVector(InputSource(it), density) }f